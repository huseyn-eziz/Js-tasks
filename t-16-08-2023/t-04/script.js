let music = prompt(`Mahni sec:
1.Ne ise
2.Öyrəşmişəm
3.Esqinden Serxoş
4.Dilbərim
`)


if (music === "1") {

    alert(`Nə isə.. 
    Axı biz əl-ələ yaşayıb öləcəkdik nə isə..
    Həyatı, ömürü yarıya böləcəkdik nə isə..
    Hələ çox görüşüb, danışıb, güləcəkdik nə isə..
    Artıq sən təksən! Üstünə xətt çəksən 
    Axı biz bir ömür yaşayıb sevəcəkdik nə isə..
    Ayrılıb, barışıb geriyə dönəcəkdik nə isə..
    Hansımız daha çox sevirik görəcəkdik nə isə..
    Artıq sən təksən!`)
}
if (music === "2") {
    alert(`Sənlə tanışan zaman 
    Heç keçməzdi ağlımdan
    Sənə belə bağlanam 
    Çıxmırsan xəyalımdan
    Bu çılğın duyğulardan
    Qurtulmaq deyil asan
    Vaxt ötür olur olan
    Alnımıza yazilan`)
}
if (music === "3") {
    alert(`Esqinnen serxos dediler bir musiqi qos
    Yanmasin ureyin derdden cos
    Dediler bir musiqi qos
    Gece sessiz kuceler bombos 
    Dediler bir musiqi qos 
    Men ne yazim gelsin ona xos
    Men ne edim gelsin ona xos`)
}
if (music === "4") {
    alert(`Bağa girmərəm sənsiz,
    Gülün dərmərəm sənsiz,
    Bağda quzu mələsə,
    Bil ki, o, mənəm sənsiz.
    
    Dilbərim, dilbərim,
    Gəl, gəl, ahu balası,
    Gəl, gəl, dağlar arası.
    Ceyranım, tərlanım,
    Gəl, gəl ahu balası,
    Uca dağlar lalası.
    
    Bu dağda bir maral var,
    Başında tirmə şal var,
    Adı çıxıb yadımdan,
    Üzündə qara xal var.`)
}